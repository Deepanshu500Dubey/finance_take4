...
def assign_cost_centers(request: CostCenterBatchRequest):
    ...
    try:
        ...
        # Check if any assignments require approval from dashboard
        pending_approvals_needed = []
        for txn_id in assignment_map:
            # Check if this transaction has a pending approval in our system
            for token, item in pending_approvals.items():
                if item.type == "Missing Cost Center" and item.metadata.get('transaction_id') == txn_id:
                    pending_approvals_needed.append({
                        "transaction_id": txn_id,
                        "approval_token": token,
                        "approval_links": get_approval_links(token)
                    })
        
        if pending_approvals_needed:
            return ToolResponse(
                success=False,
                message=f"Cannot assign cost centers - {len(pending_approvals_needed)} transactions require approval. Visit {APP_BASE_URL}/dashboard",
                data={
                    'pending_approvals': pending_approvals_needed,
                    'dashboard_url': f"{APP_BASE_URL}/dashboard"
                }
            )
...
async def get_cfo_financial_report(
    ...
    try:
        ...
        return {
            ...
            'metrics': {
                'gross_margin': ((type_totals.get('Revenue', 0) - type_totals.get('Expense', 0)) / type_totals.get('Revenue', 1)) * 100,
...
async def send_financial_reports(
    ...
    """
    Send financial reports via email with PDF/CSV attachments
    """
    ...
    html_content = request.message or f"""
    <html>
    <body>
        <h2>Financial Report - {request.fiscal_period}</h2>
        <p>Dear Finance Team,</p>
        <p>Please find attached the financial reports for period <strong>{request.fiscal_period}</strong> 
...
async def send_test_email(
    ...
    """
    Send a test email with sample reports
    """
    request = EmailReportRequest(
        recipients=[
        ...
        message=f"<h2>Test Report</h2><p>This is a test of the email reporting system.</p>"
    )
...
async def preview_email_report(
    ...
    """
    Preview what would be sent in the email (HTML preview)
    """
    ...
    <body>
        <div class="container">
            ...
            <div style="border: 1px solid #ddd; padding: 20px; border-radius: 5px;">
                ...
                <p>Dear Finance Team,</p>
                <p>Please find attached the financial reports for period <strong>{fiscal_period}</strong> 
